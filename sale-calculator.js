...
function init(){
 ...
 document.getElementById('calcCopy').addEventListener('click',async()=>{const cost=num(document.getElementById('calcCost').value), freight=num(document.getElementById('calcFreight').value), total=cost+freight; const tax=(Number(document.getElementById('calcTax').value)||0)/100, interest=(Number(document.getElementById('calcCardInterest').value)||0)/100; const baseDenom=1-margin/100-tax; const base=baseDenom>0?total/baseDenom:0; const cash=base>0?Math.ceil((base*0.95)/5)*5:0; const card3=base>0?Math.ceil(base/5)*5:0; const card12=interest>0?Math.ceil((base/(1-interest))/5)*5:card3; const v=`Preço de venda:\nÀ vista: ${br(cash)} — 5% de desconto\nCartão: ${br(card3)} — 3x sem juros\nCartão: ${br(card12)} — até 12x com juros${interest>0?' ('+(interest*100).toFixed(2).replace('.',',')+'%)':''}`; try{await navigator.clipboard.writeText(v); const b=document.getElementById('calcCopy');b.textContent='✓ Valor copiado';setTimeout(()=>b.textContent='📋 Copiar valor',1500)}catch(e){}});
 calc();
}
