...
function styleRows(){
 const id='freight-layout-v2';if(document.getElementById(id))return;
 const s=document.createElement('style');s.id=id;s.textContent=`
 .services{overflow-x:auto}.svc-head,.svc-row{grid-template-columns:minmax(220px,2.8fr) minmax(92px,1fr) minmax(92px,1fr) minmax(92px,1fr) minmax(78px,.8fr) minmax(132px,1.25fr) 34px;gap:7px;min-width:0}.svc-head{padding:9px 8px}.svc-row{padding:8px}.svc-row .freight{text-align:right}.edit-services{overflow-x:auto}.edit-svc-head,.edit-svc-row{grid-template-columns:minmax(210px,2.7fr) minmax(90px,1fr) minmax(90px,1fr) minmax(90px,1fr) minmax(76px,.8fr) minmax(125px,1.2fr) 34px;gap:7px;min-width:0}@media(max-width:850px){.svc-head,.svc-row,.edit-svc-head,.edit-svc-row{min-width:820px}}`;
 document.head.appendChild(s)
}
function ensureHeader(h){if(!h)return;const c=[...h.children],cost=c.find(x=>x.textContent.trim().toLowerCase()==='custo');if(cost&&!c.some(x=>x.classList.contains('freight-head'))){const s=document.createElement('span');s.className='freight-head';s.textContent='Frete';cost.after(s)}}
function ensureFreight(row,value){if(!row)return null;let f=row.querySelector('.freight');if(!f){f=document.createElement('input');f.className='freight money';f.inputMode='decimal';f.placeholder='R$ 0,00';const cost=row.querySelector('.cost'),tax=row.querySelector('.tax');if(cost&&tax)tax.before(f);else row.appendChild(f);f.addEventListener('input',calcNew);f.addEventListener('blur',()=>{f.value=moneyInput(f.value);calcNew()})}if(value!==undefined&&value!==null&&value!=='')f.value=moneyInput(value);return f}
...
function capture(e){const edit=e.target.closest('[data-edit]');if(edit){window.__freightEditId=edit.dataset.edit;requestAnimationFrame(()=>{const m=q('orderFixModal');if(m)m.dataset.freightOrderId=edit.dataset.edit;patchEditRows();calcEdit()})}if(e.target.closest('#add')||e.target.closest('#fixAdd'))requestAnimationFrame(()=>{patchNewRows();patchEditRows();calcNew();calcEdit()})}
function install(){
 styleRows();patchAddRow();patchNewRows();patchEditRows();
 const form=q('order');if(form&&!form.dataset.freightV2){form.dataset.freightV2='1';form.addEventListener('submit',saveNew,true);form.addEventListener('input',e=>{if(e.target.closest('#rows')){patchNewRows();calcNew()}},true)}
 const fix=q('fixEditForm');if(fix&&!fix.dataset.freightV2){fix.dataset.freightV2='1';fix.addEventListener('submit',saveEdit,true)}
 document.addEventListener('click',capture,true);
 setTimeout(()=>{patchAddRow();patchNewRows();patchEditRows();calcNew();calcEdit()},250)
}
