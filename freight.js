...
function ensureFreight(row,value){
 ...
 if(!f){
   ...
   const cost=row.querySelector('.cost'),tax=row.querySelector('.tax');
   if(cost&&tax)tax.before(f);else row.appendChild(f);
   f.addEventListener('input',calcNew);
   f.addEventListener('blur',()=>{f.value=moneyInput(f.value);calcNew()});
 }
...
function patchNewRows(){
 styleRows();ensureHeader(document.querySelector('.svc-head'));
 document.querySelectorAll('#rows .svc-row').forEach(r=>ensureFreight(r,r.dataset.freightValue));
}

function newItems(){
 return [...document.querySelectorAll('#rows .svc-row')].map(r=>({
  description:r.querySelector('.desc')?.value.trim()||'',
  sale_value:parseMoney(r.querySelector('.sale')?.value),
  cost_value:parseMoney(r.querySelector('.cost')?.value),
  freight_value:parseMoney(r.querySelector('.freight')?.value),
...
function calcNew(){
 if(!q('rows'))return;
 const a=newItems(),sale=a.reduce((s,x)=>s+x.sale_value,0),cost=a.reduce((s,x)=>s+x.cost_value,0),freight=a.reduce((s,x)=>s+x.freight_value,0),tax=a.reduce((s,x)=>s+x.sale_value*x.tax_rate/100,0);
 if(q('saleTotal'))q('saleTotal').textContent=money(sale);
 if(q('costTotal'))q('costTotal').textContent=money(cost+freight);
 if(q('taxTotal'))q('taxTotal').textContent=money(tax);
 if(q('profitTotal'))q('profitTotal').textContent=money(sale-cost-freight-tax);
 if(q('grand'))q('grand').textContent=money(sale)
}
...
async function saveNew(e){
 ...
 if(!c&&q('clientInput').value.trim()){
   const cr=await sb.from('clients').insert({company_id:company.id,name:q('clientInput').value.trim()}).select().single();
...
function patchAddRow(){
 if(typeof window.addRow!=='function'||window.addRow.__freightV3)return;
 const original=window.addRow;
 function wrapped(item={}){
   original(item);
   const r=q('rows')?.lastElementChild;
   if(r){r.dataset.freightValue=item.freight_value??'';ensureFreight(r,item.freight_value)}
   calcNew()
 }
 wrapped.__freightV3=true;window.addRow=wrapped
}

function editItems(){
 return [...document.querySelectorAll('#fixRows .edit-svc-row')].map(r=>({
  description:r.querySelector('.fd')?.value.trim()||'',
  sale_value:parseMoney(r.querySelector('.fs')?.value),
  cost_value:parseMoney(r.querySelector('.fc')?.value),
  freight_value:parseMoney(r.querySelector('.ff')?.value),
  tax_rate:Number(r.querySelector('.ft')?.value)||0,
  service_status:r.querySelector('.fst')?.value||'Pronto entregue'
 })).filter(x=>x.description||x.sale_value||x.cost_value||x.freight_value)
}

function ensureEditFreight(row,value){
 if(!row)return;
 let f=row.querySelector('.ff');
 if(!f){
   f=document.createElement('input');f.className='ff freight money';f.inputMode='decimal';f.placeholder='R$ 0,00';
   const cost=row.querySelector('.fc'),tax=row.querySelector('.ft');
   if(cost&&tax)tax.before(f);else row.appendChild(f);
   f.addEventListener('input',calcEdit);
   f.addEventListener('blur',()=>{f.value=moneyInput(f.value);calcEdit()})
 }
 if(value!==undefined&&value!==null&&value!=='')f.value=moneyInput(value)
}

function patchEditRows(){
 styleRows();ensureHeader(document.querySelector('.edit-svc-head'));
 document.querySelectorAll('#fixRows .edit-svc-row').forEach(r=>ensureEditFreight(r,r.dataset.freightValue))
}

function calcEdit(){
 if(!q('fixRows'))return;
 const a=editItems(),sale=a.reduce((s,x)=>s+x.sale_value,0),cost=a.reduce((s,x)=>s+x.cost_value,0),freight=a.reduce((s,x)=>s+x.freight_value,0),tax=a.reduce((s,x)=>s+x.sale_value*x.tax_rate/100,0);
 if(q('fixSale'))q('fixSale').textContent=money(sale);
 if(q('fixCost'))q('fixCost').textContent=money(cost+freight);
 if(q('fixTax'))q('fixTax').textContent=money(tax);
 if(q('fixProfit'))q('fixProfit').textContent=money(sale-cost-freight-tax)
}

function loadEditFreightsFromOrder(order){
 if(!order)return;
 const items=order.order_items||[];
 const rows=[...document.querySelectorAll('#fixRows .edit-svc-row')];
 rows.forEach((r,i)=>{
   const value=items[i]?.freight_value;
   if(value!==undefined&&value!==null){r.dataset.freightValue=value;ensureEditFreight(r,value)}
 });
 calcEdit()
}
...
async function saveEdit(e){
 e.preventDefault();e.stopImmediatePropagation();
 if(!company)return toast('Escolha a empresa primeiro');
 const modal=q('orderFixModal'),id=window.__freightEditId||modal?.dataset.freightOrderId;
 if(!id)return toast('Não foi possível identificar o lançamento');
 patchEditRows();const a=editItems();
 if(!a.length)return toast('Adicione pelo menos um serviço ou produto');
 const old=orders.find(x=>x.id===id);if(!old)return toast('Lançamento não encontrado');
 cloud('Salvando...');
 let c=clients.find(x=>x.name.toLowerCase()===(q('fixClient').value||'').trim().toLowerCase());
 if(!c&&q('fixClient').value.trim()){
   const cr=await sb.from('clients').insert({company_id:company.id,name:q('fixClient').value.trim()}).select().single();
   if(cr.error){cloud('Erro ao salvar',false);return toast(cr.error.message)}c=cr.data;clients.unshift(c)
 }
 ...
 const p={company_id:company.id,client_id:c?.id||old.client_id||null,entry_date:q('fixEntry').value,exit_date:q('fixExit').value,client_name:q('fixClient').value.trim(),vehicle_make_model:q('fixVehicle').value.trim(),plate:q('fixPlate').value.trim(),pedido:q('fixPedido').value.trim(),payment_status:q('fixPayment').value,total_sale:sale,total_cost:cost+freight,total_tax:tax,net_profit:sale-cost-freight-tax};
 ...
 if(dr.error){cloud('Erro ao salvar',false);return toast(dr.error.message)}
 ...
 modal.classList.add('hidden');window.__freightEditId=null;await loadData();toast('Lançamento atualizado')
}
...
function capture(e){
 ...
 if(launch){
   ...
   requestAnimationFrame(()=>requestAnimationFrame(()=>{
     ...
     if(order){
       const m=q('orderFixModal');if(m)m.dataset.freightOrderId=id;
       window.__freightEditId=id;
       patchEditRows();
       loadEditFreightsFromOrder(order)
     }
   }))
 }
}
...
function install(){
 ...
 const form=q('order');
 if(form&&!form.dataset.freightV3){
   form.dataset.freightV3='1';form.addEventListener('submit',saveNew,true);
   form.addEventListener('input',e=>{if(e.target.closest('#rows')){patchNewRows();calcNew()}},true)
 }
 const fix=q('fixEditForm');
 if(fix&&!fix.dataset.freightV3){fix.dataset.freightV3='1';fix.addEventListener('submit',saveEdit,true)}
