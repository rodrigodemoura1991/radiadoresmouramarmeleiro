...
(function(){
  ...
  function matches(order){
    ...
    const value=order.exit_date||order.entry_date;
  ...
  function apply(){
    ...
    if(info){
      if(mode==='all'){info.textContent='Todos os períodos';}
        ...
        const r=periodRange();
        info.textContent=(mode==='week'?'Semana: ':mode==='month'?'Mês: ':'Ano: ')+isoLocal(r[0])+' até '+isoLocal(r[1]);
      }
    }
  }
  function addUI(){
    ...
    wrap.id=ID;
    wrap.className='services-period-controls';
    ...
    if(date)date.value=isoLocal(new Date());
    wrap.querySelectorAll('[data-period]').forEach(btn=>btn.addEventListener('click',function(){
      mode=this.dataset.period;
      wrap.querySelectorAll('[data-period]').forEach(x=>x.classList.toggle('active',x===this));
      apply();
    }));
  ...
  function install(){
    ...
    if(typeof original==='function'&&!original.__periodFilterWrapped){
      ...
      wrapped.__periodFilterWrapped=true;
      window.renderAllServices=wrapped;
    }
  }
  let tries=0;
  const timer=setInterval(()=>{install();if(++tries>80)clearInterval(timer)},250);
