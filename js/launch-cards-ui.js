...
(function(){
  ...
  function ensureServicesCss(){
    ...
    link.id='todos-servicos-ui-css';
    link.rel='stylesheet';
    link.href='css/todos-servicos-ui.css?v=20260826-1130';
  ...
  function renderGroupedServices(){
    ...
    list.innerHTML=grouped.map(g=>{
      const o=g.order,pay=o.payment_status||'EM ABERTO',open=pay==='EM ABERTO';
      const statusClass=slug(g.items[0]?.service_status||'Liberado');
      return `<article class="service-card launch grouped-service ${statusClass} ${open?'payment-pending':'payment-'+slug(pay)}" data-order-id="${esc(o.id)}">
        <div class="grouped-top">
          ...
          <div class="grouped-main"><div class="lname">${esc(o.client_name||'Sem cliente')}</div><div class="meta">Pedido ${esc(o.pedido||'—')}${o.vehicle_make_model?' • '+esc(o.vehicle_make_model):''}${o.plate?' • '+esc(o.plate):''}</div></div>
          <b class="grouped-total">${money(o.total_sale)}</b>
        </div>
  ...
  function install(){
    ...
    if(launchList)new MutationObserver(()=>requestAnimationFrame(applyLaunches)).observe(launchList,{childList:true,subtree:true});
    const serviceList=document.getElementById('allServicesList');
    if(serviceList)new MutationObserver(()=>requestAnimationFrame(addActions)).observe(serviceList,{childList:true,subtree:true});

    /* Substitui a renderização da aba por cartões agrupados por lançamento. */
    window.renderAllServices=renderGroupedServices;
