(()=>{
  ...
  const addStyles=()=>{
    ...
    const s=document.createElement('style');s.id=STYLE_ID;s.textContent=`
      ...
      #allServicesList .service-card.payment-em-aberto,
      #allServicesList .service-card.payment-pending{background:#f5c542!important;background-color:#f5c542!important;border:2px solid #d7a900!important;border-left-color:#d7a900!important;color:#172033!important;box-shadow:0 8px 22px #d7a90033!important}
      #allServicesList .service-card.payment-em-aberto .service-main>b,
      #allServicesList .service-card.payment-em-aberto .service-date>b,
      #allServicesList .service-card.payment-pending .service-main>b,
      #allServicesList .service-card.payment-pending .service-date>b{color:#172033!important}
      #allServicesList .service-card.payment-em-aberto .service-main small,
      #allServicesList .service-card.payment-em-aberto .service-date small,
      #allServicesList .service-card.payment-pending .service-main small,
      #allServicesList .service-card.payment-pending .service-date small{color:#172033!important}
      #allServicesList .service-card.payment-em-aberto .payment-badge,
      #allServicesList .service-card.payment-pending .payment-badge{background:#fff!important;color:#173b67!important;border-color:#fff!important}
      ...
      #allServicesList .service-card.payment-falta-acertar .service-main>b,
      #allServicesList .service-card.payment-falta-acertar .service-date>b,
      #allServicesList .service-card.payment-falta-acertar .service-main small,
      #allServicesList .service-card.payment-falta-acertar .service-date small{color:#fff!important}
      ...
      /* Marca e modelo: destaque visual e negrito */
  ...
  const applyServices=()=>document.querySelectorAll('#allServicesList .service-card').forEach(card=>{
    ...
    const payment=normalize(badge?.textContent||'');
    ...
    if(small&&!small.querySelector('.service-vehicle')){
      const text=small.textContent||'';
      const marker='•';
      const pos=text.indexOf(marker);
      if(pos>=0&&text.slice(pos+1).trim()){
        const pedido=text.slice(0,pos).trim();
        const vehicle=text.slice(pos+1).trim();
        small.textContent='';
        const pedidoSpan=document.createElement('span');pedidoSpan.textContent=pedido+' ';small.appendChild(pedidoSpan);
        const vehicleSpan=document.createElement('span');vehicleSpan.className='service-vehicle';vehicleSpan.textContent='• '+vehicle;small.appendChild(vehicleSpan);
      }
    }
  });
  const start=()=>{
    addStyles();applyLaunches();applyServices();
    const launch=document.getElementById('launchList');if(launch)new MutationObserver(()=>applyLaunches()).observe(launch,{childList:true,subtree:true});
    const services=document.getElementById('allServicesList');if(services)new MutationObserver(()=>applyServices()).observe(services,{childList:true,subtree:true});
  };
