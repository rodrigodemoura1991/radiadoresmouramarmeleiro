...
async function loadData(){if(!company)return;cloud('Sincronizando...');const [o,c,p]=await Promise.all([sb.from('orders').select('*,order_items(*)').eq('company_id',company.id).order('entry_date',{ascending:false}),sb.from('clients').select('*').eq('company_id',company.id).order('name'),sb.from('catalog_items').select('*').eq('company_id',company.id).order('description')]);if(o.error||c.error||p.error){cloud('Erro na nuvem',false);toast('Não foi possível carregar os dados desta empresa: '+(o.error?.message||c.error?.message||p.error?.message||''));return}orders=o.data||[];window.orders=orders;clients=c.data||[];catalog=p.data||[];cloud('Salvo na nuvem');renderAll()}
